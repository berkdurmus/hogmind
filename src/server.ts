...
import {
  ...
  AnalyzeUserJourneySchema,
  GetMetricsSchema,
  QueryDataSchema,
  GetSessionRecordingsSchema,
  ToolResponse,
  UserJourneyAnalysis,
  MetricsAnalysis,
...
export class HogMindServer {
        ...
        tools: [
          {
            name: "get_events",
            description:
              "Retrieve PostHog events with optional filtering by date, event name, or user",
          ...
          },
          {
            name: "query_data",
            description:
              "Query PostHog data using natural language (AI-powered analysis)",
  ...
  private async handleGetEvents(args: any): Promise<ToolResponse> {
  ...
  private async handleGetInsights(args: any): Promise<ToolResponse> {
    const params = GetInsightsSchema.parse(args);

    try {
      const insights = await this.postHogService.getInsights(
        params.limit,
        params.name
      );
  ...
  private async handleCreateInsight(args: any): Promise<ToolResponse> {
  ...
  private async handleGetCohorts(args: any): Promise<ToolResponse> {
  ...
  private async handleGetPersons(args: any): Promise<ToolResponse> {
    const params = GetPersonsSchema.parse(args);

    try {
      const persons = await this.postHogService.getPersons(
        params.limit,
        params.search
      );
  ...
  private async handleGetFeatureFlags(args: any): Promise<ToolResponse> {
    const params = GetFeatureFlagsSchema.parse(args);

    try {
      const featureFlags = await this.postHogService.getFeatureFlags(
        params.active_only
      );
  ...
  private async handleAnalyzeUserJourney(args: any): Promise<ToolResponse> {
    const params = AnalyzeUserJourneySchema.parse(args);

    try {
      const events = await this.postHogService.getUserJourney(
        params.distinct_id,
        params.date_from,
        params.date_to
      );
      ...
      const keyEvents = Object.entries(eventGroups).map(
        ([event, eventList]) => ({
          event,
      ...
      insights.push(
        `User performed ${totalEvents} events across ${uniqueEvents} different event types`
      );
      insights.push(
        `Time span: ${formatDistanceToNow(parseISO(events[0].timestamp))} ago to ${formatDistanceToNow(parseISO(events[events.length - 1].timestamp))} ago`
      );
  ...
  private async handleGetMetrics(args: any): Promise<ToolResponse> {
    ...
    try {
      ...
      const topEvents = Object.entries(eventGroups)
        .map(([event, eventList]) => ({
          ...
          unique_users: new Set(
            (eventList as any[]).map((e: any) => e.distinct_id)
          ).size,
        }))
      ...
      const dailyEvents = _.groupBy(
        events,
        (event: any) => new Date(event.timestamp).toISOString().split("T")[0]
      );

      const trends = Object.entries(dailyEvents).map(([date, dayEvents]) => ({
        date,
        events: (dayEvents as any[]).length,
        users: new Set((dayEvents as any[]).map((e: any) => e.distinct_id))
          .size,
      }));

      const insights: string[] = [];
      insights.push(
        `Total of ${metrics.total_events} events from ${metrics.unique_users} unique users`
      );
      insights.push(
        `Average session duration: ${Math.round(metrics.avg_session_duration / 60)} minutes`
      );

      if (topEvents.length > 0) {
        insights.push(
          `Most popular event: "${topEvents[0].event}" with ${topEvents[0].count} occurrences`
        );
      }
  ...
  private async handleQueryData(args: any): Promise<ToolResponse> {
  ...
  private async handleGetSessionRecordings(args: any): Promise<ToolResponse> {
