...
function validateConfig() {
    ...
    if (!apiKey) {
        console.error(chalk.red("Error: POSTHOG_API_KEY environment variable is required"));
        process.exit(1);
    }
    if (!projectId) {
        console.error(chalk.red("Error: POSTHOG_PROJECT_ID environment variable is required"));
        process.exit(1);
    }
    return {
        apiKey,
...
program
    ...
    console.log(`Description: ${chalk.cyan("AI-powered analytics assistant for PostHog")}`);
    console.log(`Repository: ${chalk.blue("https://github.com/yourusername/hogmind")}`);
    console.log("");
