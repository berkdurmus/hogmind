...
export class AIService {
    ...
    static analyzeEvents(events) {
        ...
        const trends = Object.entries(eventGroups).map(([event, eventList]) => {
            const previousCount = Math.max(1, eventList.length * 0.8); // Simulate previous period
            const change = ((eventList.length - previousCount) / previousCount) * 100;
            return {
                metric: event,
                ...
                direction: change > 5
                    ...
                    : change < -5
                        ? "down"
                        : "stable",
                significance: Math.abs(change) > 20
                    ? "high"
                    : Math.abs(change) > 10
                        ? "medium"
                        : "low",
        ...
        const summary = `Analyzed ${totalEvents} events across ${uniqueEventTypes} event types from ${uniqueUsers} unique users. ` +
            `Average of ${avgEventsPerUser} events per user over ${timeRange}.`;
        // Generate recommendations
        const recommendations = this.generateRecommendations(events, eventGroups, userGroups, trends);
        return {
            summary,
    ...
    static parseNaturalLanguageQuery(query) {
        ...
        const parameters = {};
        // Extract date ranges
        const dateMatches = lowerQuery.match(/(?:last|past)\s+(\d+)\s+(day|week|month)s?/);
        if (dateMatches) {
        ...
        else if (lowerQuery.includes("metric") ||
        ...
        else if (lowerQuery.includes("feature flag") ||
            lowerQuery.includes("flag")) {
            intent = "feature_flags";
            suggestedAction = "get_feature_flags";
        }
        else if (lowerQuery.includes("recording") ||
    ...
    static analyzeUserBehavior(events) {
        ...
        const userEventCounts = Object.entries(userGroups).map(([userId, userEvents]) => ({
            userId,
        ...
        const highEngagementUsers = userEventCounts.filter((u) => u.eventCount > 20);
        if (highEngagementUsers.length > 0) {
            patterns.push({
                pattern: "high_engagement",
                confidence: Math.min(100, (highEngagementUsers.length / userEventCounts.length) * 100),
        ...
        if (powerUsers.length > 0) {
            patterns.push({
                pattern: "power_users",
                confidence: Math.min(100, (powerUsers.length / userEventCounts.length) * 100),
        ...
        if (inactiveUsers.length > 0) {
            patterns.push({
                ...
                confidence: Math.min(100, (inactiveUsers.length / userEventCounts.length) * 100),
     ...
     */
    static generateOptimizationSuggestions(events, insights) {
        const suggestions = [];
        const eventGroups = _.groupBy(events, "event");
        const userGroups = _.groupBy(events, "distinct_id");
        ...
        if (funnelEvents.length > 1) {
            ...
            for (let i = 1; i < funnelCounts.length; i++) {
                const conversionRate = (funnelCounts[i].users / funnelCounts[i - 1].users) * 100;
    ...
    static getTimeRange(events) {
        ...
        if (daysDiff < 30)
            return `${Math.round(daysDiff / 7)} weeks`;
        return `${Math.round(daysDiff / 30)} months`;
    }
    static calculateTimeSpan(events) {
        if (events.length < 2)
            return 0;
        const sortedEvents = events.sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
        return differenceInDays(parseISO(sortedEvents[sortedEvents.length - 1].timestamp), parseISO(sortedEvents[0].timestamp));
    }
    static generateRecommendations(events, eventGroups, userGroups, trends) {
        const recommendations = [];
        const totalEvents = events.length;
        const totalUsers = Object.keys(userGroups).length;
        // Event volume recommendations
        if (totalEvents < 1000) {
            recommendations.push("Consider implementing more tracking to capture additional user interactions");
        }
        // User engagement recommendations
        const avgEventsPerUser = totalEvents / totalUsers;
        if (avgEventsPerUser < 5) {
            recommendations.push("Focus on user onboarding to increase engagement and event frequency");
        }
        else if (avgEventsPerUser > 50) {
            recommendations.push("High user engagement detected - consider analyzing power user behaviors for growth insights");
        }
        // Event distribution recommendations
        const topEvent = Object.entries(eventGroups).sort(([, a], [, b]) => b.length - a.length)[0];
        if (topEvent && topEvent[1].length > totalEvents * 0.5) {
            recommendations.push(`"${topEvent[0]}" represents majority of events - consider tracking more diverse actions`);
        }
        ...
        const growingTrends = trends.filter((t) => t.direction === "up" && t.significance === "high");
        if (growingTrends.length > 0) {
            recommendations.push(`Capitalize on growing trends in: ${growingTrends.map((t) => t.metric).join(", ")}`);
        }
        // Default recommendation if no specific insights
        if (recommendations.length === 0) {
            recommendations.push("Data looks healthy - continue monitoring key metrics and user behavior patterns");
        }
